...
# update code to also talk to an ACTIONTEC C1000A
...
def enter_user(user):
    usernameElement = driver.find_element_by_xpath("//input[@id='admin_user_name']")
...
def get_dsl_status():
    ...
    while ctr < 6:
        try:
            #                                                # //span[@id='Upstream']
            dslUpstreamElement = driver.find_element_by_xpath("//span[@id='Upstream']")
            ...
            print("DSL status is  NOT yet there")
            ctr += 1
            time.sleep(1)

    if ctr == 0:
        #                                                    //span[@id='Downstream']
        dslDownstreamElement = driver.find_element_by_xpath("//span[@id='Downstream']")
        ...
        # the c1100a does not provide DSL status ... i think the modem light, doesn't show-up as a variable
...
def get_dsl_transport():
    ...
    while ctr < 4:
        try:
            #                                                  //span[@id='rxPacket']
            Packets_Downstream = driver.find_element_by_xpath("//span[@id='rxPacket']")
    ...
    if ctr == 0:
        #                                                //span[@id='txPacket']
        Packets_Upstream = driver.find_element_by_xpath("//span[@id='txPacket']")
        ...
        #                                                      //span[@id='rxTotalstream']
        Total_Usage_Downstream = driver.find_element_by_xpath("//span[@id='rxTotalstream']")
        ...
        #                                                    //span[@id='txTotalstream']
        Total_Usage_Upstream = driver.find_element_by_xpath("//span[@id='txTotalstream']")
...
def get_dsl_channel():
    ...
    #                                      //span[@id='Near_endRS']
    RS_FEC = driver.find_element_by_xpath("//span[@id='Near_endRS']")
    #RS_FEC = driver.find_element_by_xpath("//td[@id='msDslChannelTble_@RS_FEC_Correction_1']")
    sample['RS_FEC'] = int(RS_FEC.text)

    #                                          //span[@id='Near_endCRC']
    CRC_Errors = driver.find_element_by_xpath("//span[@id='Near_endCRC']")
...
def get_dsl_power():
    ...
    while ctr < 6:
        try:
            # DOWNSTREAM ....                              //span[@id='snr_margin']
            SNR_downstream = driver.find_element_by_xpath("//span[@id='snr_margin']")
            #SNR_downstream = driver.find_element_by_xpath("//td[@id='msDslPowerTble_@SNR_1']")
            sample['SNR_downstream'] = float(SNR_downstream.text)
    ...
    if ctr == 0:
        # UPSTREAM ....                              //span[@id='snr_margin_u']
        SNR_upstream = driver.find_element_by_xpath("//span[@id='snr_margin_u']")
        #SNR_upstream = driver.find_element_by_xpath("//td[@id='msDslPowerTble_@SNR_2']")
        sample['SNR_upstream'] =  float(SNR_upstream.text)
        #sample['SNR_upstream'] =  float(refp(SNR_upstream.text))

        #                                              # //span[@id='down_power']
        Power_downstream = driver.find_element_by_xpath("//span[@id='down_power']")
        #Power_downstream = driver.find_element_by_xpath("//td[@id='msDslPowerTble_@Power_1']")
        sample['Power_downstream'] = float(Power_downstream.text)
        #sample['Power_downstream'] = float(refp(Power_downstream.text))

        #                                              //span[@id='up_power']
        Power_upstream = driver.find_element_by_xpath("//span[@id='up_power']")
        #Power_upstream = driver.find_element_by_xpath("//td[@id='msDslPowerTble_@Power_2']")
        sample['Power_upstream'] =  float(Power_upstream.text)
...
def get_dsl_link():
    ...
    while ctr < 4:
        try:
            LinkUptime = driver.find_element_by_xpath("//span[@id='retains_time']")
    ...
    if ctr == 0:
        LinkTrainErrors = driver.find_element_by_xpath("//span[@id='adslInitErr']")
...
def get_internet_settings():
    ...
    while ctr < 6:
        try:
            internetConnection = driver.find_element_by_xpath("//span[@id='ISP_status2']")
    ...
    #time.sleep(2)
    if ctr == 0 :
        #                                           //span[@id='session_time']
        sessionTime = driver.find_element_by_xpath("//span[@id='session_time']")
        sample['sessionTime'] =  sessionTime.text

        #                                           //span[@id='Packet_sent']
        packetsSent = driver.find_element_by_xpath("//span[@id='Packet_sent']")
        sample['packetsSent'] = int(packetsSent.text)
        
        #                                               //span[@id='Packet_rece']
        packetsReceived = driver.find_element_by_xpath("//span[@id='Packet_rece']")
...
def gotoInternet():
    ...
    while ctr < 4:
        try:
            ...
            #print("we are in Internet screens???")
            time.sleep(1)
...
def gotoUtilities():
    ...
    while ctr < 6:
        try:
            # //a[contains(text(),'Utilities')]
            #                                                   //a[contains(text(),'Utilities')]
            #goUtilities_button = driver.find_element_by_xpath("//a[contains(text().'Utilities')]").click()
            goUtilities_button = driver.find_element_by_link_text("Utilities").click()
            time.sleep(5)
            return 1
        except NoSuchElementException as Exception:
            print("trying again, to goto Utilities")
            ctr += 1
            time.sleep(1)
    return -1
...
def domodemStatusMain():
    ...
    while ctr < 6:
        try:
            gomodemStatus_button = driver.find_element_by_xpath("//a[contains(text(),'Modem Status')]").click()
            #goDsl1Status_button = driver.find_element_by_xpath("//li[@class='modemstatus']").click()
            #print("called modem status okay???")
            return 1
        except NoSuchElementException as Exception:
...
def gotoDsl1():
    ...
    while ctr < 6:
        #print("gotoDsl***")
        try:
            # //a[@id='wanstatus']                              //a[@id='wanstatus']
            goDsl1Status_button = driver.find_element_by_xpath("//a[@id='wanstatus']").click()
            #goDsl1Status_button = driver.find_element_by_xpath("//a[@id='dslstatus-1']").click()
            return 1
        except NoSuchElementException as Exception:
            print("trying DSL again")
            ctr += 1
            time.sleep(1)
    return -1
...
def maybeClearDsl1(clear):
    ...
    if clear == 'yes':
#                                                    //a[@class='btn clear_btn']
        clear_button = driver.find_element_by_xpath("//a[@class='btn clear_btn']").click()
        time.sleep(2)
        #                                               //a[@class='dialog-ok_new']
        popup_ok_button = driver.find_element_by_xpath("//a[@class='dialog-ok_new']").click()
...
def maybeReboot(reboot):
    ...
    #time.sleep(1)
    if reboot == 'yes':
        #                                           # //a[@class='btn reboot_btn']
        reboot_button = driver.find_element_by_xpath("//a[@class='btn reboot_btn']").click()
...
def doLoginButton():
    ...
    login_button = driver.find_element_by_xpath("//a[@id='apply_btn']").click()
...
def doLogoutButton(reboot):
    if reboot == 'yes' :
        ...
        # //div[@id='logout_btn']
        # //a[@class='btn reboot_btn']
        logout_button = driver.find_element_by_xpath("//div[@id='logout_btn']").click()
        return 0
...
def gotoResourcetable():
    ...
    while ctr < 4:
        try:
            #                                                      //a[@id='modemutilization']
            goresourcetable_button = driver.find_element_by_xpath("//a[@id='modemutilization']").click()
...
def get_resourcetable(ts):
    ...
    while thing_size == 0:
        counter += 1
        #                                      //strong[contains(text(),'LAN Device Log')]
        aThing = driver.find_element_by_xpath("//strong[contains(text(),'LAN Device Log')]")
        thing_size = len(aThing.text)
        time.sleep(1)

    aTable = driver.find_element_by_xpath("//table[@id='utilityLog']")
